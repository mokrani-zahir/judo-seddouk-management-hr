...
- **Piège CRLF (IMPORTANT)** : GitHub stocke les fichiers texte en LF et `raw.githubusercontent.com` les sert en LF → un `.bat` pendant les fins de ligne LF est mal interprété par cmd (Win7 surtout) → la fenêtre s'ouvre puis **se ferme aussitôt**. Correction : `.gitattributes` avec `win7/*.bat -text` pour que le blob reste en CRLF `i/crlf`. Vérification : `git ls-files --eol win7/install_Win7.bat` doit afficher `i/crlf`. L'URL `master` peut être servie d'après un cache proxy ; l'URL épinglée par SHA (`.../{SHA}/win7/...`) est sans cache. Après chaque modif du .bat, vérifier par curl que raw sert bien `CRLF>0`.
